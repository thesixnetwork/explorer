      ...
      <b-col>
        <b-form-group label="Available Token" label-for="Token">
          <validation-provider
            ...
            name="Token"
          >
            <b-form-select v-model="token" @change="tokenChange">
              <template #first>
...
export default {
  ...
  computed: {
    destinationOptions() {
      ...
      if (this.token.startsWith('ibc/')) {
        ...
        return query
          ? options.filter(x => x.channel_id === query.channel_id)
          : options;
      }
      return options;
    },
      ...
      return [
        {
          ...
          value: {
            ...
            token: coin(
              Number(getUnitAmount(this.amount, this.token)),
              this.token
            ),
            sender: this.address,
  ...
  methods: {
    loadData() {
      ...
      if (this.address) {
        this.$http.getIBCChannels(this.selectedChain, null).then(ret => {
          const chans = ret.channels
            .filter(x => x.state === 'STATE_OPEN')
            .map(x => ({ channel_id: x.channel_id, port_id: x.port_id }));
    ...
    onChannelChange() {
      this.$http
        .getIBCChannelClientState(
          this.destination.channel_id,
          this.destination.port_id,
          this.selectedChain
        )
        .then(cs => {
