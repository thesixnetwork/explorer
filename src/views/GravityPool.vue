<template>
  <div class="container-md">
    <b-row class="match-height">
      <b-col v-for="(data, index) in pools.pools" :key="index" md="4" sm="6">
            ...
            <b-row>
              <b-col cols="8">
                <b-card-title class="mb-1 text-uppercase">
                  #{{ data.id }}
                  {{ formatDenom(data.reserve_coin_denoms[0]) }} -
                  {{ formatDenom(data.reserve_coin_denoms[1])
                  }}<small class="font-small-2"> xx</small>
                </b-card-title>
...
export default {
  ...
  methods: {
    formatDenom(v) {
      const denom = v.startsWith('ibc')
        ? this.ibcDenom[v].denom_trace.base_denom
        : v;
